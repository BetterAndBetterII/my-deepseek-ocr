  ...
  const refreshUsage = useCallback(async () => {
    ...
    try {
      const [s, l] = await Promise.all([
        usageSummary(token || undefined),
        usageList(token || undefined),
      ]);
      setSummary(s);
      setHistory(l);
    } catch {}
    ...
    <div className="mx-auto max-w-6xl p-4 grid gap-4">
              ...
              <div className="border rounded-md overflow-hidden flex flex-col min-h-0">
                {isPdf ? (
                  <PdfStreamViewer
                    ...
                    onChangePage={(p, next) =>
                      setPages((arr) =>
                        arr.map((it) => (it.page === p ? { ...it, content: next } : it)),
                      )
                    }
                    onCopyAll={() => {
      ...
      {insightsOpen && (
        <div className="fixed inset-0 z-20" onClick={() => setInsightsOpen(false)}>
          ...
          <div
            className="absolute right-4 top-20 w-[320px] max-h-[70vh] overflow-auto rounded-xl border bg-background/70 backdrop-blur shadow-lg"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-4 py-3 border-b flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm">
                <BarChart3 className="h-4 w-4" /> 用量与记录
              </div>
              <button
              ...
              <div>
                ...
                {summary ? (
                  <div className="text-xs grid grid-cols-2 gap-x-3 gap-y-1">
                    <div className="text-muted-foreground">总次数</div>
                    <div>{formatNumber(summary.total_events)}</div>
                    <div className="text-muted-foreground">输入</div>
                    <div>
                      {formatBytes(summary.total_input_bytes)} (
                      {formatNumber(summary.total_input_bytes)})
                    </div>
                    <div className="text-muted-foreground">提示字符</div>
                    <div>{formatNumber(summary.total_prompt_chars)}</div>
                    <div className="text-muted-foreground">结果字符</div>
                    <div>{formatNumber(summary.total_completion_chars)}</div>
                    <div className="text-muted-foreground">提示 Token</div>
                    <div>{formatNumber(summary.total_prompt_tokens)}</div>
                    <div className="text-muted-foreground">结果 Token</div>
                    <div>{formatNumber(summary.total_completion_tokens)}</div>
                  </div>
                ) : (
