...
export function MetricsPills({ summary }: { summary: UsageSummary | null }) {
  ...
  const items = [
    ...
    { icon: Cpu, label: "Token", value: `${formatNumber(summary.total_prompt_tokens + summary.total_completion_tokens)}` },
  ];
  return (
    <div className="flex gap-2">
      {items.map(({ icon: Icon, label, value }, idx) => (
        <div key={idx} className="flex items-center gap-2 rounded-full border bg-background/60 backdrop-blur px-3 py-1 text-xs">
